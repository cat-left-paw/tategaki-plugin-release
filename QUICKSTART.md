...
- **執筆モード（新方式）**：Obsidian の Markdown を直接編集（変換・同期なし）
    - 参照モード機能も統合され、Obsidian のノートと自動連携します
- **互換モード（TipTap版）**：v1.1.1以前の執筆モード（変換・同期あり、現在も利用可能）
    - 将来のアップデートで廃止する可能性があるため、継続利用中なら新方式への移行をおすすめします
...
> **補足**: 互換モード（TipTap版）は現在も利用できますが、将来のアップデートで廃止する可能性があります。継続利用している場合は、（新）執筆モードへの移行をおすすめします。
