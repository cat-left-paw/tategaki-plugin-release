...
- `<=1.2.1` から更新する場合は、`README(.ja).md` と `INSTALL.md` の移行手順を先に確認してください

このプラグインには主に 2 つの編集モードがあります。

- **執筆・参照モード**：Obsidian の Markdown を直接編集（変換・同期なし）
    - 参照表示機能も統合され、Obsidian のノートと自動連携します
- **互換モード**：v1.1.1以前の執筆モード（変換・同期あり、現在も利用可能）
    - 将来のアップデートで廃止する可能性があるため、継続利用中なら執筆・参照モードへの移行をおすすめします
...
執筆・参照モードでは、Obsidian の Markdown を直接編集します。**変換・同期の処理が不要**なため、従来の同期リスクがありません。
...
執筆・参照モード内で、Obsidian のノートをアクティブにすると、自動的に参照表示（縦書きビューを確認しながら Obsidian のノートを編集する状態）に切り替わります。
...
- **リストをアウトライナー風に整理**
    - 横書き: `Ctrl` / `Cmd` + `↑` / `↓`
    - 縦書き: `Ctrl` / `Cmd` + `→` / `←`
    - `Tab` / `Shift+Tab` でネスト変更
    - 番号付きリストは連番を自動補正
- **リスト・引用ブロックの Enter / Shift+Enter**
    - `Enter` で継続入力や終了を自然に処理
    - `Shift+Enter` で項目内改行や空行継続
...
- 保存場所: `<Vault設定フォルダ>/tategaki-sync-backups/`（通常は `.obsidian/tategaki-sync-backups/`）
...
> **補足**: 互換モードは現在も利用できますが、将来のアップデートで廃止する可能性があります。継続利用している場合は、執筆・参照モードへの移行をおすすめします。
