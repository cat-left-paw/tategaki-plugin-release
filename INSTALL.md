...
このプラグインは 現在のところ、**コミュニティプラグイン未登録**のため、Obsidian 内の「参照」から検索してインストールすることはできません。  
...
展開したフォルダにあるインストーラーを実行し、Vault フォルダを選ぶだけでインストールできます。（ZIP のままではインストーラーは起動しないので、必ず展開（解凍）すること）
...
1. インストーラーのセキュリティの設定を「許可する」に変更する
    - Windows フォルダ内の`install.ps1` を右クリック → プロパティ → セキュリティ：このファイルは他のコンピューターから..（略）..ブロックされる可能性があります → **許可する** にチェック → 適用　 → OK
2. `install.ps1` を右クリック「PowerShell で実行」を選ぶ
3. 表示されるダイアログで **Vault フォルダ** を選ぶ（`.obsidian` の外側）
4. 完了メッセージが出たら、下の「有効化」へ進む
...
1. インストーラーのセキュリティの設定を「許可する」に変更する
    - Windows フォルダ内の`install-helper.cmd`を右クリック → プロパティ → セキュリティ：このファイルは他のコンピューターから..（略）..ブロックされる可能性があります → **許可する** にチェック → 適用　 → OK
2. `install-helper.cmd` を右クリック → **管理者として実行**
3. 表示されるダイアログで **Vault フォルダ** を選ぶ（`.obsidian` の外側）
4. 完了メッセージが出たら、下の「有効化」へ進む
...
※macOS の Gatekeeper で警告が出る場合は、次の方法を試してください。

1.  Finder で `Mac/install.command` を右クリック → 「開く」
2.  システム設定 →「プライバシーとセキュリティ」→「このまま開く」を選択
