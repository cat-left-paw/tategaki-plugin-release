...
- **互換モード**: v1.1.1以前の執筆モード（TipTap版）は「互換モード」として引き続き利用できます（設定で有効化が必要／将来的に整理予定）
- **モバイル非対応**: このバージョンからデスクトップのみ対応とします
...
- コミュニティプラグイン未登録（近いうちに申請予定）のため、Obsidian 標準の自動アップデートは使えません
- モバイルは非対応（デスクトップのみ）
  - どうしてもタブレット等で使いたい場合は、`manifest.json` の `isDesktopOnly` を `false` に変更し、PC側のプラグインと同期して導入することで動く可能性があります
  - ただし **動作保証・サポート対象外** です
...
- **CE補助(IME)**（（新）執筆・参照モードのみ）: 入力やカーソル移動がうまくいかないときの補助モードです。オンにすると IME 入力が安定する場合があります（もう一度押すとオフになります）
...
**注意**: 互換モードは将来的に整理予定です。（新）執筆・参照モードの使用をおすすめします。
