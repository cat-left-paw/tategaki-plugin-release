...
- **互換モード**: v1.1.1以前の執筆モード（TipTap版）は「互換モード」として引き続き利用できますが、将来のアップデートで廃止する可能性があります
- **段落単位のソーステキスト編集を改良**: SoT 上で、現在の段落だけを Markdown 記号込みで直接編集しやすくなりました
- **SoT の選択モードを追加**: クリック位置の反映を優先する `クリック優先` と、ドラッグ選択の自然さを優先する `ドラッグ選択優先` を選べます
...
- **SoT の選択モード**：クリック優先 / ドラッグ選択優先を選択可能
...
- **表示設定**: フォント/サイズ/余白など、見た目の設定を開きます（テーマ管理もここから）
    - SoT の選択モード、IME表示補正、書籍モード設定、プラグイン設定へのショートカットもここから扱えます
- **アウトライン**: 見出し一覧を表示し、クリックでその見出しへジャンプできます

#### 段落単位のソーステキスト編集

SoT の「ソーステキスト編集」は、現在の段落だけを Markdown 記号込みで直接編集するための機能です。
...
（新）執筆・参照モードでは、リストと引用で次のように動作します。
...
**注意**: 互換モード（TipTap版）は現在も利用できますが、将来のアップデートで廃止する可能性があります。継続利用している場合は、（新）執筆・参照モードへの移行をおすすめします。
